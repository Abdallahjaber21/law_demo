...
class GdWatermark {
     ...
     */
    protected static function imageCopyMergeAlpha(&$dst_im, &$src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h) {
        ...
        imagecopy($dst_im, $cut, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h);
    }
     ...
     */
    public static function addWatermark(&$wm, $pos, $offsetX, $offsetY, &$that) {
        ...
        if (preg_match('/right|east/i', $pos)) {
            $wmPosX += $picDim['width'] - $wmDim['width'];
        } else if (!preg_match('/left|west/i', $pos)) {
            $wmPosX += intval($picDim['width'] / 2 - $wmDim['width'] / 2);
        }

        if (preg_match('/bottom|lower|south/i', $pos)) {
            $wmPosY += $picDim['height'] - $wmDim['height'];
        } else if (!preg_match('/upper|top|north/i', $pos)) {
            $wmPosY += intval($picDim['height'] / 2 - $wmDim['height'] / 2);
        }

        $workingImage = $that->getWorkingImage();
        $wmImage = ($wm->getWorkingImage() ? $wm->getWorkingImage() : $wm->getOldImage());

        static::imageCopyMergeAlpha($workingImage, $wmImage, $wmPosX, $wmPosY, 0, 0, $wmDim['width'], $wmDim['height']);
