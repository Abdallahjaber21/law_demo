...
class MultilingualUrlManager extends UrlManager {
  ...
  public function beforeAction($event) {
    if (/* !Yii::$app->errorHandler->exception && */ count($this->languages) > 1) {
      ...
      $this->updateCurrentUserLanguage();
    }

    return $event->isValid;
  }

  private function updateCurrentUserLanguage() {
    if (!Yii::$app->getUser()->isGuest) {
      $user = Yii::$app->getUser()->getIdentity();
      if (isset($user->language)) {
        if ($user->language != Yii::$app->language) {
  ...
  public function createUrl($params) {
    ...
    if (count($this->languages) > 0) {
      ...
      if (!isset($params['language'])) {
        if (!empty(Yii::$app->session) && Yii::$app->session->has('language')) {
          $language = Yii::$app->session->get('language');
        } elseif (!empty(Yii::$app->cookies) && isset(Yii::$app->request->cookies['language'])) {
          $language = Yii::$app->request->cookies['language']->value;
        } else {
